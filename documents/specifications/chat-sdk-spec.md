...
- [ ] 사용자 프로필 관리
  ...
  - 프로필 정보 조회
- [ ] WebSocket 연결/재연결 (자동 재연결 포함)
- [ ] 연결 상태 모니터링 (연결됨, 연결 중, 끊김)
- [ ] 로그아웃 및 연결 해제
...
### 1.2 채널(채팅방) 관리
- [ ] 1:1 채팅 채널 생성
- [ ] 그룹 채팅 채널 생성 (⚙️ 기본 최대 3명, Config 설정 가능)
- [ ] 채널 목록 조회 (페이징 지원)
- [ ] 채널 정보 조회 (참여자, 생성 시간 등)
- [ ] 채널 나가기
- [ ] 채널 삭제 (⚙️ Config 설정 가능)
  - 1:1 채팅:
...
### 1.3 메시지 송수신
- [ ] 텍스트 메시지 전송
- [ ] 이미지 전송 (압축 옵션, 최대 20MB)
- [ ] 파일 전송 (문서/기타, 최대 10MB)
- [ ] 영상 전송: 지원 안 함 (Phase 1~2)
- [ ] 메시지 수신 실시간 알림
- [ ] 메시지 히스토리 조회 (페이징)
- [ ] 메시지 수정 (⚙️ 기본 10분 이내, Config 설정 가능)
- [ ] 메시지 삭제 (본인 메시지만, "삭제된 메시지입니다" 표시)
- [ ] 읽지 않은 메시지 카운트

**구현 우선순위**: 
- 텍스트, 이미지, 히스토리: 🔴 Phase 1
- 파일 전송, 수정/삭제: 🟡 Phase 2
